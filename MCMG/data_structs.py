...
def canonicalize_smiles_from_list(smiles_list):
    """Reads a SMILES file and returns a list of RDKIT SMILES"""
    smiles = []
    for i, line in enumerate(smiles_list):
        if i % 100000 == 0:
            print("{} lines processed.".format(i))
        smi = line.split(",")[0]
    ...
    print("{} SMILES retrieved".format(len(smiles)))
    return smiles
...
def construct_vocabulary(smiles_list, voc_path='data/moses/Voc_RE'):
...
def main(args):
    ...
    smiles_list = canonicalize_smiles_from_list(df_data['SMILES'])
    print("Constructing vocabulary...")
    voc_chars = construct_vocabulary(smiles_list, args.output_dir + "Voc_RE")
    write_smiles_to_file(smiles_list, args.output_dir + "mols_filtered.smi")


def parse_args():
    parser = argparse.ArgumentParser(description='Convert data into suitable format',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--input_file', '-t', type=str, help='Full path to SMILES file containing training data')
