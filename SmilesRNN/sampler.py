...
def _sample_batch(model: SmilesRnn, batch_size: int, device, max_seq_length, temperature) -> torch.Tensor:
    ...
    for char in range(max_seq_length):
        logits, hiddens = model(x, hiddens)
...
def sample(model: SmilesRnn, num_to_sample=10000, device='cpu', batch_size=64, max_seq_length=140, temperature=1.0, seed=42):
