...
def _sample_batch(model: CharRNN, batch_size: int, device, max_seq_length, temperature) -> torch.Tensor:
    ...
    for char in range(max_seq_length):
        logits, _, hiddens = model.forward(x, lens, hiddens)
...
def sample(model: CharRNN, num_to_sample=10000, device='cpu', batch_size=64, max_seq_length=140, temperature=1.0, seed=42):
