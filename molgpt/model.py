...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.type_emb = nn.Embedding(2, config.n_embd)  # xux: add several more embeddings here
        if config.num_props:
        ...
        if config.lstm:  # xux: add lstm for scaffold embedding?
    ...
    def configure_optimizers(self, train_config):
        ...
        decay = set()
        no_decay = set()
        whitelist_weight_modules = (torch.nn.Linear, torch.nn.LSTM)  # xux: init add LSTM
        ...
        optimizer = torch.optim.AdamW(optim_groups, lr=train_config.learning_rate, betas=train_config.betas)  # xux: use AdamW for training
        return optimizer

    def forward(self, idx, targets=None, prop = None, scaffold = None):  # xux: add 2 more properties prop & scaffold
        b, t = idx.size()
        ...
        type_embeddings = self.type_emb(torch.ones((b,t), dtype = torch.long, device = idx.device))  # xux: help gpt to recognize task type
        x = self.drop(token_embeddings + position_embeddings + type_embeddings)

        if self.config.num_props:  # xux: add embedded props as dimensions of x
        ...
        # if self.config.scaffold:  # xux: add embedded scaffolds as dimensions of x
        #     type_embd = self.type_emb(torch.zeros((b, 1), dtype = torch.long, device = idx.device))
        ...
        for layer in self.blocks:  # retrieve attention maps from network
            x, attn = layer(x)
        ...
        #     num = 1
        #     logits = logits[:, num:, :]  # xux: jump over first column? start token?
