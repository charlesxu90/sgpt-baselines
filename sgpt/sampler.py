...
from model import SGPT
from dataset import SmilesCharDictionary, rnn_start_token_vector

sd = SmilesCharDictionary()


def _sample_batch(model: SGPT, batch_size: int, device, max_seq_length, temperature) -> torch.Tensor:
    x = rnn_start_token_vector(batch_size, device)
    indices = torch.zeros((batch_size, max_seq_length), dtype=torch.long).to(device)
    lens = torch.tensor([1 for _ in range(batch_size)], dtype=torch.long, device='cpu')
    hiddens = None

    for char in range(max_seq_length):
        logits, _, hiddens = model(x, lens, hiddens)
...
def sample(model: SGPT, num_to_sample=10000, device='cpu', batch_size=64, max_seq_length=140, temperature=1.0, seed=42):
