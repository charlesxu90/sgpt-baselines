...
from trainer import SGPTTrainer
from dataset import load_smiles_from_list, get_tensor_dataset
...
def train(train_data, val_data, save_dir, device='cuda', max_len=140, n_epochs=10):
	print('Training...')
	...
	model = SGPT()
	trainer = SGPTTrainer(model, save_dir=save_dir, n_epochs=n_epochs, device=device)
	trainer.fit(train_data=train_set, val_data=test_set)
	return model
